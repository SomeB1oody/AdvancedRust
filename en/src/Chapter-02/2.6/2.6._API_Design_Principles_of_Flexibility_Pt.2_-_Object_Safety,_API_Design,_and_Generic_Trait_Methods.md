...
## 2.6.1. Object Safety
When defining a trait, whether it is object-safe is also part of the unstated contract (see [2.5.1. Code Contracts](../2.5/2.5._API_Design_Principles_of_Flexibility_Pt.1_-_Contracts_and_More_Flexible_Interfaces_with_Generic_Parameters.md)).
...
1. **All supertraits must also be object-safe**  
   If a trait inherits from other traits (supertraits, see [Rust Guide 19.2.5. Using Supertraits to Require Additional Trait Functionality](https://someb1oody.github.io/RustGuide/en/Chapter-19/19.2/19.2._Advanced_Traits.html)), then those supertraits must also be object-safe.

2. **It must not require `Sized`**  
   A trait cannot use `Sized` (see [Rust Guide 19.5.4. Dynamically Sized Types and the `Sized` Trait](https://someb1oody.github.io/RustGuide/en/Chapter-19/19.5/19.5._Advanced_Types.html)) as a supertrait, meaning it cannot contain a `Self: Sized` bound, because the size of a trait object is unknown at compile time.
...
If a trait must have a generic method, consider putting the generic parameter on the trait itself (see [1.16.2. Generic (Type-Parameter) Traits](../../Chapter-01/1.16/1.16._Generic_Traits_-_Generic_(Type-Parameter)_Traits_and_Associated-Type_Traits.md)).
