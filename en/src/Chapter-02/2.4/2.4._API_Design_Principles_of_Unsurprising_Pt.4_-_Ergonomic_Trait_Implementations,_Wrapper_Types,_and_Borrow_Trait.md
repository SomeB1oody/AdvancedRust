...
To solve this, when defining a new trait, we usually provide corresponding blanket implementations (see [1.17.2. Blanket Implementation](../../Chapter-01/1.17/1.17._Orphan_Rules,_Coherence,_and_Consistency_-_Blanket_and_Covered_Implementations.md)) for the following (when the trait methods allow it—typically methods that take `&self` or `&mut self`):
