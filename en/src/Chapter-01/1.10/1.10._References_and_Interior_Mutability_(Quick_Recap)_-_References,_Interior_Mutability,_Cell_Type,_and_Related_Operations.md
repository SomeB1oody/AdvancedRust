...
This article is only a quick recap of references and interior mutability. For ownership, see [1.9. Ownership (Quick Recap)](../1.9/1.9._Ownership_(Quick_Recap)_-_Core_Ideas_of_Ownership,_How_to_Implement_Copy_Trait,_Value_Drop,_and_Drop_Order.md).

## 1.10.1. References
Through references, Rust allows values to be borrowed without giving up ownership.

**A reference is a pointer with an additional contract attached.** Rust has two kinds of references. (For the difference between pointers and references, see [1.1. Pointer Overview (Part 1)](../1.1/1.1._Pointer_Overview_(Part_1)_-_What_Is_a_Pointer,_and_the_Difference_Between_Pointers_and_References.md).)
