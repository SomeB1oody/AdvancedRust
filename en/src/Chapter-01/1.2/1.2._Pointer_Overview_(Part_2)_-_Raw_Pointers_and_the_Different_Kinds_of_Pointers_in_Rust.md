...
- Raw pointers can ignore the borrow rules by allowing *both mutable and immutable pointers at the same time* or *multiple mutable pointers to the same location* (see [4.4. References and Borrowing](../../Chapter-04/4.4/4.4._References_and_Borrowing.md) for the borrow rules).
