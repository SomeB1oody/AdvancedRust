...
## 1.4.2. Stack Memory
There is an axiom: “When in doubt, prefer the stack.” But if you want to put data on the stack, the compiler must know the size of the type. In other words: “When in doubt, prefer a type that implements `Sized`.” (For details on the `Sized` trait, see [Rust Guide 19.5.4. Dynamically Sized Types and the `Sized` Trait](https://someb1oody.github.io/RustGuide/en/Chapter-19/19.5/19.5._Advanced_Types.html).)
