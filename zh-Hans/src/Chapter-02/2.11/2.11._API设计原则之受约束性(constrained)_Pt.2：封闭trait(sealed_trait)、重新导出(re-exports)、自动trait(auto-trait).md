...
### 重新导出(re-exports)
重新导出(re-exports)的操作在 [【Rust自学】14.3.1. 使用`pub use`导出方便使用的API](https://blog.csdn.net/weixin_71793197/article/details/145293174) 中有过介绍。

如果你的接口的某部分暴露了外部类型，那么外部类型的任何更改也将成为你接口的变更。

最好用newtype（详见 [【Rust自学】19.2.6. 使用newtype模式在外部类型上实现外部trait](https://blog.csdn.net/weixin_71793197/article/details/145418117)）包裹外部类型，仅仅暴露外部类型中你认为有用的部分。
